...
local UnitSpecific = {
    ...
    player = function(self, ...)
		...
		if G.myClass == "MONK" or G.myClass == "MAGE" then
			...
			for i = 1, 4 do
				totembars[i] = T.createStatusbar(totembars, "ARTWORK", C["UnitframeOptions"]["height"], 3, 1, 1, 1, 1, G.uiname.."Totem1")
		...
		-- Stagger
		if G.myClass == "MONK" and C["UnitframeOptions"]["stagger"] then
			local stagger = T.createStatusbar(self, "ARTWORK", C["UnitframeOptions"]["height"]*-(C["UnitframeOptions"]["hpheight"]-1), nil, 1, 1, 1, 1)
		...
		-- Shaman mana
		if multicheck(G.myClass, "SHAMAN", "PRESIT", "DRUID") and C["UnitframeOptions"]["dpsmana"] then
			local dpsmana = T.createStatusbar(self, "ARTWORK", C["UnitframeOptions"]["height"]*-(C["UnitframeOptions"]["hpheight"]-1), nil, 1, 1, 1, 1)
