...
local CustomFilter = function(...)
	...
	if C["CooldownAura"]["Debuffs"][name] then
		priority = C["CooldownAura"]["Debuffs"][name].level
	elseif IsInInstance() then
        local ins = GetInstanceInfo()
        if C["RaidDebuff"][ins] then
			for boss, debufflist in pairs(C["RaidDebuff"][ins]) do
...
oUF:AddElement("MlightAuras2", Update, Enable, Disable)